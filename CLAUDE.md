...
**誰可以註冊**由 `REGISTER_CODE` 決定：沒設 = 開放註冊（預設），有設 = 要填對
邀請碼。這個服務的讀取端點大多是公開的，所以多一個帳號洩漏不了東西；真正的
成本是**有人可以無限開帳號並上傳光表**佔掉資料庫。部署在公開 IP 上時建議設一組。
...
- **2026-08-16**：**登入頁可以自己建立帳號**（`POST /api/register` + Login 頁的
  ...
  `REGISTER_CODE` 沒設就是開放註冊，設了就要填邀請碼。
